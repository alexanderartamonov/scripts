...
def assume_aws_threedsuat():
    sts_connection_3ds_uat = boto3.client('sts')
    threeds_uat_acct = sts_connection_3ds_uat.assume_role(
        RoleArn="arn:aws:iam::089465505731:role/CloudWatchMonitoring",
        RoleSessionName="cross_acct_3dsuat_lambda"
    )
    
    ACCESS_KEY = threeds_uat_acct['Credentials']['AccessKeyId']
    SECRET_KEY = threeds_uat_acct['Credentials']['SecretAccessKey']
    SESSION_TOKEN = threeds_uat_acct['Credentials']['SessionToken']
    ec2_3ds_uat_client = boto3.client(
        'ec2',
    ...
    return ec2_3ds_uat_client
    
def assume_aws_management():
    sts_connection_mgmt = boto3.client('sts')
    mgmt_acct = sts_connection_mgmt.assume_role(
        RoleArn="some management account role arn",
        RoleSessionName="cross_acct_mgmt_lambda"
    )
    
    ACCESS_KEY = mgmt_acct['Credentials']['AccessKeyId']
    SECRET_KEY = mgmt_acct['Credentials']['SecretAccessKey']
    SESSION_TOKEN = mgmt_acct['Credentials']['SessionToken']
    ec2_mgmt_client = boto3.client(
        'ec2',
        aws_access_key_id=ACCESS_KEY,
        aws_secret_access_key=SECRET_KEY,
        aws_session_token=SESSION_TOKEN,
    ...
    if 'Records' in event and isinstance(event['Records'], list):
        for record in event['Records']:
            if 'Sns' in record and 'Message' in record['Sns']:
                ...
                ec2_3ds_uat_client = assume_aws_threedsuat() # Assumes 3ds uat role
                private_ip = sns_message_ip
                response = ec2_3ds_uat_client.describe_instances(
                        Filters=[{
